...
export class Store {
  ...
  createShape(element: ShapeEditorElement) {
    ...
    switch (element.properties.shapeType) {
      ...
      case "square":
        return new fabric.Rect({ ...options, width: 100, height: 100 });
      case "rectangle":
        return new fabric.Rect({ ...options, width: 150, height: 100 });
      case "triangle":
        return new fabric.Triangle({ ...options, width: 100, height: 100 });
      case "parallelogram":
  ...
  addShapeResource(shapeType: string, x=0, y=0) {
    const id = getUid();
    const newShape = {
       id,
       name: `Shape ${id}`,
    ...
    this.addEditorElement(newShape);
 }
 

  addImage(index: number, x = 0, y = 0) {
  ...
  exportToJson() {
    ...
    const editorState = {
      canvas: this.canvas.toJSON(['id', 'name', 'customFilter']), // Include custom properties if needed
      elements: this.editorElements,
    ...
    return JSON.stringify(editorState, null, 2); // Pretty print the JSON
  }
  // saveCanvasToVideo() {
  ...
  refreshElements() {
    ...
    for (let index = 0; index < store.editorElements.length; index++) {
      ...
      switch (element.type) {
        ...
        case "shape": {
          let fabricShape = this.createShape(element);
          element.fabricObject = fabricShape;
          canvas.add(fabricShape);
          canvas.on("object:modified", function (e) {
            if (!e.target) return;
            const target = e.target;
            if (target != fabricShape) return;
            const placement = element.placement;
            const newPlacement: Placement = {
              ...placement,
              x: target.left ?? placement.x,
